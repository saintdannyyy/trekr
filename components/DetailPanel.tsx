...
export default function DetailPanel({
  ...
  const [tab, setTab] = useState<
    "overview" | "interviews" | "contacts" | "documents" | "reminders"
  >("overview");
    ...
    >
      <div className="w-full max-w-md bg-white border-l border-stone-200 shadow-xl flex flex-col h-full overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-stone-100 flex items-start justify-between">
          <button
            onClick={onClose}
            className="text-stone-400 hover:text-stone-600 text-sm flex items-center gap-1 mt-0.5"
          >
            ← back
          </button>
          ...
          <>
            ...
            <div className="px-6 py-5 border-b border-stone-100">
              ...
              <div className="flex items-center gap-3 mt-3 flex-wrap">
                <StatusBadge
                  status={app.status}
                  customStatus={app.custom_status}
                />
                {app.work_type && (
            ...
            <div className="flex border-b border-stone-100 px-6 overflow-x-auto">
              {(
                ...
                >
                  {t}
                  {t === "interviews" && app.interview_rounds?.length
                    ? ` (${app.interview_rounds.length})`
                    : ""}
                  {t === "contacts" && app.contacts?.length
                    ? ` (${app.contacts.length})`
                    : ""}
                  {t === "documents" && app.documents?.length
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 text-sm">
              ...
              {tab === "interviews" && (
                <div className="space-y-3">
                  {!app.interview_rounds?.length && (
                    <Empty text="No interview rounds logged yet." />
                  )}
              ...
              {tab === "contacts" && (
                <div className="space-y-3">
                  {!app.contacts?.length && (
                    <Empty text="No contacts logged yet." />
                  )}
                  {app.contacts?.map((c) => (
                    <div key={c.id} className="card p-4">
                      <p className="font-medium text-stone-800">{c.name}</p>
                      {c.title && (
                        <p className="text-xs text-stone-400">{c.title}</p>
                      )}
                      ...
                      {c.notes && (
                        <p className="text-stone-500 mt-2 text-xs">{c.notes}</p>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {tab === "documents" && (
                <div className="space-y-3">
                  {!app.documents?.length && (
                    <Empty text="No documents logged yet." />
                  )}
                  {app.documents?.map((d) => (
                    <div
                      key={d.id}
              ...
              {tab === "reminders" && (
                <div className="space-y-3">
                  ...
                  {app.reminders?.map((r) => (
                    <div
                      key={r.id}
                      className={`card p-4 ${r.done ? "opacity-50" : ""}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p
                            className={`font-medium ${r.done ? "line-through text-stone-400" : "text-stone-800"}`}
                          >
                            {r.message}
                          </p>
                          <p className="text-xs text-stone-400 mt-0.5">
                        ...
                        {!r.done && (
                          <span className="text-[10px] bg-brand-soft text-brand border border-brand/20 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                            Pending
                          </span>
                        )}
                      </div>
...
function OutcomePill({ outcome }: { outcome?: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${styles[outcome] ?? styles.Pending}`}
    >
      {outcome}
    </span>
  );
